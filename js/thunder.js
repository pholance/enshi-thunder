...
var echart1, echart2;
function init() {
    echart1 = echarts.init(document.getElementById('hour'));
    echart1.setOption({
        xAxis: [{
            type: 'category',
    ...
    echart2 = echarts.init(document.getElementById('month'));
    echart2.setOption({
        xAxis: [{
            type: 'category',
...
function detailReport(cloud) {
    ...
    cloud.forEach(function (t, number) {
        hour[t.hour]++;
    });
    var hourPoint = new Array(24);
    for (var i = 0; i < 24; i++) {
        hourPoint[i] = {
            value: hour[i],
            xAxis: i,
...
function parseGrid(data) {
    ...
    AMap.event.addListener(search, "complete", function (e) {
        ...
        AMap.event.addListener(grid, "click", function (e) {
            ...
            $('#container').off("click", 'button#detail');
            $('#container').on('click', 'button#detail', function () {
                detailReport(grid.getExtData());
            })
        });
    });
